...
# input_label.pack()
# input_text = tk.Text(win, height=5, width=53)
# input_text.pack()
...
def save_translation():
    try:
        with open("translation_history.txt", "a", encoding="utf-8") as file:
            ...
            file.write(input_text.get("1.0", tk.END).strip() + "\n")
            file.write("Translated Text:\n")
            file.write(output_text.get("1.0", tk.END).strip() + "\n")
            file.write("-" * 50 + "\n")
        output_text.insert(tk.END, "✅ Translation saved to 'translation_history.txt'\n")
    except Exception as e:
        output_text.insert(tk.END, f"⚠️ Error saving translation: {e}\n")

# Buttons
tk.Button(win, text="Start Translation", command=run_translator).place(relx=0.25, rely=0.9, anchor="c")
tk.Button(win, text="Kill Execution", command=kill_execution).place(relx=0.5, rely=0.9, anchor="c")
tk.Button(win, text="About this project", command=open_about_page).place(relx=0.75, rely=0.9, anchor="c")
tk.Button(win, text="Save Translation", command=save_translation).place(relx=0.5, rely=0.96, anchor="c")
